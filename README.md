...
A desktop password, card, and secure-note manager built with Tkinter. Each user gets a themed vault with optional OTP, clipboard helpers, and encrypted storage using Argon2id + AES-256-GCM.
...
- Secure notes: create named note files with descriptions, edit in-app, and save encrypted content (Ctrl+S supported).
...
- Encryption: keys derived with Argon2id; vault lines and notes encrypted with AES-256-GCM.
...
## Usage
1. Launch and **Register** a user (creates `files/<user>/` with encrypted vault files, `files/<user>/notes/`, and `config/settings.json`).
2. **Login**; if OTP is enabled, enter the 6-digit code. If a PIN is required, you will be prompted.
3. Use the left menu to switch between **Logins**, **Cards**, **Notes**, and **Settings**.
4. Click **+** to add entries. Passwords can be generated and toggled between hidden/visible.
5. Click a note entry to open it, edit text, and press Ctrl+S (or use save) to encrypt and store the note.
6. Click fields in the detail view to copy (username/password/TOTP/card fields). Use the eye/edit/bin icons to show, edit, or delete.
7. In **Settings**, adjust theme, default screen, favicon loading, password visibility, PIN requirement, and OTP toggle.

## Data & Encryption
- User data lives in `files/<user>/`.
- Vault files (`passwords.txt`, `cards.txt`) store encrypted lines: AES-256-GCM ciphertext with a per-line nonce.
- Note files are saved per entry in `files/<user>/notes/<name>-<desc>.txt`, encrypted with AES-256-GCM using the same vault key.
